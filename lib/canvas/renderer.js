...
function drawElement(ctx, el, theme) {
  ...
  switch (el.type) {
    ...
    case ELEMENT_TYPES.GRAPH_EDGE:       drawGraphEdge(ctx, el); break;
    default: break;
  }
...
function drawArrow(ctx, el) {
  ...
  const headLen = 12;
  ctx.fillStyle = style.stroke ?? "#6b7280";
  ctx.beginPath();
  ctx.moveTo(x2, y2);
...
function drawGraphEdge(ctx, el) {
  ...
  if (data.weight !== undefined) {
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(String(data.weight), mx + 10, my - 10);
  }
}
