...
function tokenize(str) {
  let clean = str.trim();
  // Strip outer array brackets if present (e.g. "[1, 2, 3]" -> "1, 2, 3")
  if (clean.startsWith("[") && clean.endsWith("]")) {
    clean = clean.slice(1, -1).trim();
  }
  // If user entered a bracket/character sequence like "{[()]}" or "()[]{}" without spaces
  if (clean.length > 1 && !clean.includes(" ") && !clean.includes(",") && /^[()\[\]{}]+$/.test(clean)) {
    return clean.split("");
  }
  return clean
    .split(/[\s,]+/)
...
export function parseArray2D(str) {
  ...
  if (clean.startsWith("[[")) {
    ...
    try {
      ...
      rows = parsed.map((row) => row.map((v) => (isNaN(v) ? v : Number(v))));
    } catch {
      throw new Error('Invalid 2D array. Use format [[1,2],[3,4]]');
    }
  } else {
    rows = clean.split(/[\n/]+/).map((row) =>
      row.replace(/[\[\]]/g, "").split(/[\s,]+/).filter(Boolean).map((t) => (isNaN(t) ? t : Number(t)))
    );
  }
...
export function parseGraph(str, options = { directed: false, weighted: false }) {
  let clean = str.trim();
  // Strip potential variable assignment prefix: e.g. "graph = " or "adj = "
  clean = clean.replace(/^(?:(?:const|let|var)\s+)?[a-zA-Z0-9_$]+\s*=\s*/, "").trim();
  ...
  const addNode = (id) => {
    ...
    if (!nodes.has(sId)) {
      nodes.set(sId, {
        id: sId,
        label: isNaN(sId) ? sId : Number(sId),
        numericKey: isNaN(sId) ? null : Number(sId),
      });
    }
    return sId;
  };

  const addEdge = (from, to, weight = null) => {
    if (from === null || to === null || from === undefined || to === undefined) return;
    const f = addNode(from);
    const t = addNode(to);
    edges.push({
      from: f,
      to: t,
      weight: options.weighted ? (weight !== null && !isNaN(weight) ? Number(weight) : 1) : null,
      directed: options.directed ?? false,
    });
  };

  const finalizeGraph = () => {
    if (!nodes.size) throw new Error("Invalid graph input. Try: 1 2 or 1 - 2 : 5");
  ...
  // 1. Try parsing JSON/Python array of pairs: e.g. [[1,2],[2,3]] or [[0,1,5],[1,2,3]]
  if (clean.startsWith("[") && clean.endsWith("]")) {
    try {
      const arr = JSON.parse(clean.replace(/'/g, '"'));
      if (Array.isArray(arr) && arr.every((item) => Array.isArray(item))) {
        arr.forEach((pair) => {
          if (pair.length >= 2) addEdge(pair[0], pair[1], pair[2]);
        });
        if (nodes.size > 0) return finalizeGraph();
      }
    } catch {}
  }

  // 2. Try parsing Python/JSON dict adjacency list: e.g. {1: [2, 3], 2: [1, 3]} or {"A": ["B", "C"]}
  if (clean.startsWith("{") && clean.endsWith("}")) {
    try {
      const dictStr = clean.replace(/'/g, '"');
      const obj = JSON.parse(dictStr);
      if (typeof obj === "object" && obj !== null && !Array.isArray(obj)) {
        Object.entries(obj).forEach(([u, neighbors]) => {
          addNode(u);
          if (Array.isArray(neighbors)) {
            neighbors.forEach((v) => {
              if (Array.isArray(v) && v.length >= 2) addEdge(u, v[0], v[1]);
              else addEdge(u, v);
            });
          }
        });
  ...
  // 3. Line or comma/semicolon separated statements
  const rawSegments = clean
    .split(/[\n;]+/)
    .flatMap((line) => {
      // If line contains colon adjacency list (e.g. "1: 2, 3" or "A: B C")
      const colonAdjMatch = line.match(/^([^-–→>:\s]+)\s*:\s*([^:]+)$/);
      if (colonAdjMatch) {
        const u = colonAdjMatch[1].trim();
        const rest = colonAdjMatch[2].trim();
        // Check if rest is "2:5" (weighted edge) vs "2, 3, 4" (neighbor list)
        if (!rest.includes(":") && !/^\s*\d+\s*$/.test(rest)) {
          const neighbors = rest.replace(/[,\[\]]+/g, " ").split(/\s+/).filter(Boolean);
          if (neighbors.length > 0) {
            return neighbors.map((v) => `${u} ${v}`);
          }
        }
      }
  ...
  for (const segment of rawSegments) {
    ...
    // Match "0-1(2)", "0 - 1 (2)", "0->1[2]", "0 1 (2)"
    const parenMatch = segment.match(/^([^-–→>:\(\[\s]+)\s*[-–→>\s]*\s*([^-–→>:\(\[\s]+)\s*[\(\[]\s*(-?\d+(?:\.\d+)?)\s*[\)\]]/);
    // Match "1 - 2 : 5" or "1-2:5" or "1 -> 2 : 5"
    const colonMatch = !parenMatch && segment.match(/^([^-–→>:\s]+)\s*[-–→>\s]+\s*([^-–→>:\s]+)\s*:\s*(-?\d+(?:\.\d+)?)/);

    if (parenMatch) {
      from = parenMatch[1].trim();
      to = parenMatch[2].trim();
      weight = Number(parenMatch[3]);
    } else if (colonMatch) {
